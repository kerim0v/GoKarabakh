...
function BookingModal({ booking, onClose, onSubmit }) {
  ...
  return <div className="auth-backdrop-enhanced booking-backdrop" style={{ zIndex: 2000 }} onClick={(event) => event.target === event.currentTarget && onClose()}>
    <div className="auth-card-enhanced glass booking-card" style={{ maxWidth: "580px", width: "min(92vw, 580px)" }} role="dialog" aria-modal="true" aria-labelledby="booking-title">
...
const readCachedKx = () => {
  try { return JSON.parse(localStorage.getItem("karabakhUser") || "{}").kx_count || 0; } catch { return 0; }
};
...
function Header({ active }) {
  ...
  const [balance, setBalance] = useState(readCachedKx);

  useEffect(() => {
    const syncAuth = () =>
      setIsLoggedIn(localStorage.getItem("isLoggedIn") === "true");
    const syncCoins = () => setBalance(readCachedKx());
    window.addEventListener("auth:changed", syncAuth);
    window.addEventListener("auth:changed", syncCoins);
    window.addEventListener("coins:changed", syncCoins);
    return () => {
      window.removeEventListener("auth:changed", syncAuth);
      window.removeEventListener("auth:changed", syncCoins);
...
function Profile() {
  ...
  useEffect(() => {
    const sync = () => { setIsLoggedIn(localStorage.getItem("isLoggedIn") === "true"); setUser(readJson("karabakhUser", {})); };
  ...
  useEffect(() => {
    ...
    api.fetchMe()
      ...
      .catch(() => {});
    api.getKxTransactions().then(setTransactions).catch(() => {});
  ...
  const balance = user.kx_count || 0;
  const earned = transactions.filter((t) => t.amount > 0).reduce((total, t) => total + t.amount, 0);
  const spent = Math.abs(transactions.filter((t) => t.amount < 0).reduce((total, t) => total + t.amount, 0));
  const userRole = user.role || "user";
  const roleLabel = userRole === "owner" ? "Business Owner" : userRole === "guide" ? "Guide" : userRole === "admin" ? "Admin" : "User";
...
function CommunityArchive() {
  ...
  useEffect(() => {
    ...
    if (isLoggedIn) {
      ...
      api.getKxTransactions()
        .then((txs) => setRewardClaimed(txs.some((t) => t.reason === "Shared your first community trace")))
        .catch(() => {});
    }
  }, [isLoggedIn]);
  ...
  const share = async (event) => {
    ...
    try {
      ...
      const justEarnedBonus = !rewardClaimed;
      const freshMe = await api.fetchMe();
      setMe(freshMe);
      localStorage.setItem("karabakhUser", JSON.stringify(freshMe));
