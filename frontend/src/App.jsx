...
import { haveAccount, User } from "./js/user.js";
  ...
  useEffect(() => {
    const fetchCoins = async () => {
      ...
      if (userID && userID !== "undefined") {
        ...
        setCoins(user.karabakhCoins.toString());
      } else {
...
function DistrictPage({ slug }) {
  ...
  const updatePlaces = async (places) => {
    ...
    await user.updatePlaces(places);
  ...
  useEffect(() => {
    const fetchPlaces = async () => {
      const id = localStorage.getItem("loginID");
      const user = await User.fromId(id);
      setSavedPlaces(await user.getPlaces());
    };
    fetchPlaces();
  }, []);
                      ...
                      onClick={async () =>
                        await updatePlaces(
                          savedPlaces.includes(title)
                            ? savedPlaces.filter((place) => place !== title)
                            : [...savedPlaces, title],
                        )
                      }
                    >
